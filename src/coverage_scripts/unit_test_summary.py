...
try:
    ...
    test_summary = "**Test Summary**: {} Passed, {} Failed, {} skipped, {} errors in {} seconds\n".format(
        tests - failures - skipped - errors, failures, skipped, errors, time_seconds
    )
    print(test_summary)

    job_link = 'https://buildkite.com/uber/generic-udj/builds/' + os.getenv('BUILDKITE_BUILD_NUMBER')

    # Write the test summary to a file
    with open('phabricator-comment.txt', 'w') as comment_file:
        comment_file.write(f"More information can be found [here]({job_link}).\n")
        comment_file.write(test_summary)
        if failures > 0:
            comment_file.write("**Warning**: There are test failures!\n\n\n")
