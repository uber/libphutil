...
MDX = """ SELECT NON EMPTY
 {[Line of Business].[Delivery], [Line of Business].[Mobility]} * {[Ops Metric].[Cash Trips], [Ops Metric].[Surged Trips], [Ops Metric].[Shopping Sessions]}
 ON 0,
 NON EMPTY
 {[Period].[%s]} * {TM1FILTERBYLEVEL( {TM1SUBSETALL( [Rate Type] )}, 0)}
 ON 1
FROM
    [Ops]
 WHERE
...
class Mytest(Validation):
    name = 'Ops-Surged-Cash_Trips&Shop_see_Zero values'
    email_to = []
    ...
    def prepare(self):
        ...
        # Form the MDX based on day
            ## Surged Trips and Delivery Combinations are always Zero so Reduce the total Fixed Zero Cells

        TOTAL_RECORD_COUNT = (Num_Rate_Elements * 2 * 3) - Num_Rate_Elements
