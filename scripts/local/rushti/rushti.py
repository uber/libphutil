...
def setup_tm1_services():
    ...
    for tm1_server_name, params in config.items():
        ...
        if tm1_server_name != config.default_section:
            try:
                params["password"] = tm1get_credentials(service_name)
...
# recieves two arguments: 1) path-to-txt-file, 2) max-workers 3) service_name
if __name__ == "__main__":
    ...
    path_to_file, max_workers, service_name = translate_cmd_arguments(*sys.argv)
    # setup connections
